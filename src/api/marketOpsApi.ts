...
export async function getSupplierIntegrations(){const{data}=await api.get("/suppliers/integrations");return data as Array<{provider:string;name:string;configured:boolean;mode:"API"|"MANUAL"|"CONFIGURATION_REQUIRED";capabilities:{catalog:boolean;prices:boolean;stock:boolean;dropshipping:boolean;orderQuote:boolean;orderCreate:boolean;shippingGuide:boolean;tracking:boolean}}>;}
export async function searchSupplierCatalog(provider:string,search:string,page=1,limit=30){const{data}=await api.get("/suppliers/catalog/search",{params:{provider,q:search,page,limit,stock:true}});return data as{provider:string;query:string;page:number;limit:number;total:number;products:Array<{provider:string;providerProductId:string;sku:string;title:string;brand?:string|null;category?:string|null;price?:number|null;currency?:string|null;stock?:number|null;imageUrl?:string|null}>};}
export async function importSupplierProduct(payload:{provider:string;providerProductId:string;sku?:string;title:string;brand?:string|null;category?:string|null;price?:number|null;currency?:string|null;stock?:number|null;imageUrl?:string|null;dropshippingEnabled?:boolean}){const{data}=await api.post("/suppliers/catalog/import",payload);return data as{product:Product;supplierProduct:Record<string,unknown>};}
