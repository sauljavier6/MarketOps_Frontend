...
export default function RadarPage() {
  ...
  return <div>
    ...
    {selected && <section className="dashboard-grid radar-detail"><article className="panel"><div className="panel-header"><div><p className="eyebrow">INVESTMENT ANALYSIS</p><h2>{selected.Title}</h2><p className="subtitle">{selected.Season || (evidence.sourceStrategy === "EVERGREEN" ? "Evergreen" : "Sin temporada")} · {stageLabel(evidence.stage)}</p></div><div className="form-actions"><button className="secondary" onClick={() => setOfferModal(true)}><Plus size={14}/> Cotización proveedor</button><button className="secondary" onClick={() => setCostModal(true)}><Plus size={14}/> Costos de venta</button></div></div>
      ...
      <button className="primary full-width" disabled={!canFinalize || recommend.isPending} onClick={calculateFinal}><Calculator size={15}/>{recommend.isPending ? "Calculando..." : !offers.data?.length ? "Falta cotización de proveedor" : !sellingCosts?.verified ? "Faltan costos de venta" : "Calcular decisión final"}</button>
      {recommend.isError && <div className="form-error">{recommend.error.message}</div>}
    </article><article className="panel recommendation-panel"><p className="eyebrow">DECISIÓN</p>{!finalResult ? <div className="recommendation-empty"><h2>{decisionLabel(evidence.decision)}</h2><p>{evidence.decisionReason || "MarketOps seguirá en RESEARCH hasta que todos los datos críticos estén disponibles."}</p>{recommendation && <div className="recommendation-metrics"><span>Capital disponible<b>{money(recommendation.availableCapital)}</b></span><span>Reserva objetivo<b>{money(recommendation.reserveTarget)}</b></span><span>Unidades<b>{recommendation.quantity || 0}</b></span><span>Inversión<b>{money(recommendation.investment)}</b></span></div>}</div> : <div><div className={`decision ${String(finalResult.decision).toLowerCase()}`}>{finalResult.decision}</div><h2>{finalResult.title}</h2><p>{finalResult.reason}</p><div className="recommendation-metrics"><span>Proveedor<b>{finalResult.supplierName || "—"}</b></span><span>Costo puesto<b>{money(finalResult.economics?.landedCost)}</b></span><span>Comprar<b>{finalResult.recommendedQuantity} u.</b></span><span>Invertir<b>{money(finalResult.recommendedInvestment)}</b></span><span>Utilidad/u<b>{money(finalResult.economics?.unitProfit)}</b></span><span>Margen<b>{pct(finalResult.economics?.netMarginPct)}</b></span><span>ROI<b>{pct(finalResult.economics?.roiPct)}</b></span><span>Score<b>{finalResult.investmentScore}</b></span></div></div>}</article></section>}
    ...
    <Modal open={offerModal} title="Cotización verificada" onClose={() => setOfferModal(false)}><form className="form-grid" onSubmit={submitOffer}><label className="span-2">Proveedor<input name="supplier" required/></label><label>Fuente<input name="source" placeholder="Mayorista / fabricante"/></label><label>URL<input name="url"/></label><label>Precio unitario<input name="price" type="number" step=".01" required/></label><label>MOQ<input name="moq" type="number" min="1" defaultValue="1"/></label><label>Envío total de compra<input name="shipping" type="number" min="0" step=".01" defaultValue="0"/></label><label>Importación / otros compra<input name="import" type="number" min="0" step=".01" defaultValue="0"/></label><label>Lead time total (días)<input name="days" type="number" min="0" defaultValue="7"/></label><label>Confianza proveedor<input name="reliability" type="number" min="0" max="100" defaultValue="70"/></label><div className="form-actions span-2"><button className="primary">Guardar cotización</button></div></form></Modal>
    <Modal open={costModal} title="Costos de venta verificados" onClose={() => setCostModal(false)}><form className="form-grid" onSubmit={submitSellingCosts}><div className="source-note span-2">Captura únicamente costos que conozcas o hayas validado. MarketOps no los inventa; estos valores habilitan la economía y la decisión final.</div><label>Envío Mercado Libre / unidad<input name="marketplaceShipping" type="number" min="0" step=".01" required defaultValue={sellingCosts?.marketplaceShipping ?? 0}/></label><label>Empaque / unidad<input name="packagingCost" type="number" min="0" step=".01" required defaultValue={sellingCosts?.packagingCost ?? 0}/></label><label>Otros costos de venta / unidad<input name="otherSellingCosts" type="number" min="0" step=".01" required defaultValue={sellingCosts?.otherSellingCosts ?? 0}/></label><div className="form-actions span-2"><button className="primary">Confirmar costos</button></div></form></Modal>
  </div>;
}
